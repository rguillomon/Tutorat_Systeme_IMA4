...
void config_periph(){
    ...
    for(int num_intf=0;num_intf < config->bNumInterfaces ;num_intf++){
        ...
        int interface; //on récupère ttes les numéros des alt_settings numéro 0 => for et détach
        
        interface=config->interface[num_intf].altsetting[0].bInterfaceNumber;

        // on detach cette interface
        if(libusb_kernel_driver_active(handle,interface)){
        int status=libusb_detach_kernel_driver(handle,config->interface);
